...
try:
    ...
    st.info(f"""
    **🔍 Diagnóstico Táctico Estructural:**
    * **Situación de Fibonacci:** El precio cotiza entre el soporte de **${soporte_proximo:.2f}** y la resistencia de **${resistencia_proxima:.2f}**. 
    * **Techo Clave:** Un quiebre confirmado del máximo reciente en **${max_fib:.2f}** proyecta un objetivo por Extensión de Fibonacci hacia **${ext_1618:.2f}** (+{((ext_1618-precio_actual)/precio_actual)*100:.1f}%).
    * **Estructura Chartista:** {'El activo muestra fortaleza alcista manteniéndose sobre la Zona Dorada (61.8%).' if precio_actual >= fib_618 else 'El activo ha penetrado niveles clave de retroceso, lo que sugiere cautela antes de abrir nuevas posiciones.'}
    """)
...
try:
    ...
    # --- CÁLCULO DE FIBONACCI (Últimos 252 días de rueda / 1 Año) ---
    df_recent = df_d.tail(252)
    max_fib = df_recent['High'].max()
    min_fib = df_recent['Low'].min()
    diff_fib = max_fib - min_fib
    ...
    fib_618 = max_fib - 0.618 * diff_fib
    fib_100 = min_fib
    ext_1618 = max_fib + 0.618 * diff_fib  # Resistencia de extensión superior
    ...
    # --- NUEVA SECCIÓN: INTERPRETACIÓN DE ESTRUCTURAS Y FIBONACCI ---
    st.subheader(f"📐 Interpretación de Estructuras y Niveles Clave ({ticker_sel})")
    
    # Determinar próxima resistencia y próximo soporte según Fibonacci
    niveles_fib = [fib_100, fib_618, fib_500, fib_382, fib_236, fib_0, ext_1618]
    resistencia_proxima = min([n for n in niveles_fib if n > precio_actual], default=ext_1618)
    soporte_proximo = max([n for n in niveles_fib if n < precio_actual], default=fib_100)
    ...
    st.info(f"""
    **🔍 Diagnóstico Táctico Estructural:**
    * **Situación de Fibonacci:** El precio se encuentra cotizando entre el soporte de **${soporte_proximo:.2f}** y la resistencia de **${resistencia_proxima:.2f}**. 
    * **Techo Clave:** Un quiebre confirmado del máximo reciente en **${max_fib:.2f}** proyecta un objetivo por Extensión de Fibonacci hacia **${ext_1618:.2f}** (+{((ext_1618-precio_actual)/precio_actual)*100:.1f}%).
    * **Estructura Chartista:** {'El activo muestra fortaleza alcista manteniéndose sobre la Zona Dorada (61.8%).' if precio_actual >= fib_618 else 'El activo ha penetrado niveles clave de retroceso, lo que sugiere cautela antes de abrir nuevas posiciones.'}
    """)
    ...
    # --- GRÁFICO DE VELAS CON NIVELES DE FIBONACCI TRAZADOS ---
    st.subheader("📈 Gráfico de Velas (5 Años) con Estructura de Fibonacci")
    fig = make_subplots(rows=3, cols=1, shared_xaxes=True, vertical_spacing=0.03, row_heights=[0.6, 0.2, 0.2])
    ...
    # Líneas de Fibonacci en el Gráfico Principal
    fig.add_hline(y=max_fib, line_dash="dash", line_color="red", annotation_text=f"Fib 0% (Máx ${max_fib:.1f})", row=1, col=1)
    fig.add_hline(y=fib_382, line_dash="dash", line_color="orange", annotation_text=f"Fib 38.2% (${fib_382:.1f})", row=1, col=1)
    fig.add_hline(y=fib_618, line_dash="dash", line_color="gold", annotation_text=f"Fib 61.8% (${fib_618:.1f})", row=1, col=1)
    fig.add_hline(y=min_fib, line_dash="dash", line_color="green", annotation_text=f"Fib 100% (Mín ${min_fib:.1f})", row=1, col=1)
